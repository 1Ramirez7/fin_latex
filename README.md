...
## Making virtual python enviroment

```bash
python -m venv .venv  # venv is the systax that makes the env. the “ .venv or .env “ will be the name of the enveriment folder.
py -3.13 -m venv .venv # to force a specific build
.venv\Scripts\activate # to activate env (windows)
```

## Dependency Setup

```bash
pip install -r requirements.txt # run this first if txt file already present.
pip install -r requirements-dev.txt # optional if running dev-tools like pip-tools
python -m pip install jupyter # install libraries/packages example
```
...
### Edit dependency source files manually as needed

* Runtime: `requirements.in`
* Dev tools: `requirements-dev.in`
  * Only add packages/dev-tools project/repo import/depend-on.
...
```
* txt file answers: “Exactly what versions make this repo work in current environment?”
